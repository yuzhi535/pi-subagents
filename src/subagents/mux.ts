...
async function _zellijActionAsync(
	args: string[],
	surface?: string,
): Promise<string> {
...
let _cmuxSubagentPane: string | null = null;
...
function _createSurfaceInPane(name: string, pane: string): string {
	const out = execSync(
		`cmux new-surface --pane ${shellEscape(pane)} --focus true`,
		{
			encoding: "utf8",
		},
	).trim();
...
export function createSurfaceSplit(
	...
	if (backend === "cmux") {
		const surfaceArg = fromSurface
			? ` --surface ${shellEscape(fromSurface)}`
			: "";
		const out = execSync(
			`cmux new-split ${direction}${surfaceArg} --focus true`,
			{
				encoding: "utf8",
			},
		).trim();
...
export function interruptSurface(surface: string): void {
	...
	if (backend === "cmux") {
		execSync(
			`cmux send --surface ${shellEscape(surface)} ${shellEscape("\u0003")}`,
			{
				encoding: "utf8",
			},
		);
	...
	if (backend === "tmux") {
		execFileSync("tmux", ["send-keys", "-t", surface, "C-c"], {
			encoding: "utf8",
		});
		return;
	}
...
export function consumeSubagentExitSignal(
	sessionFile: string,
): PollResult | null {
